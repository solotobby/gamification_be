...
if (!function_exists('setProfile')) {
    ...
    {
        $ip = (env('APP_ENV') == 'local') ? '102.89.68.65' : request()->ip();

        $location = Location::get($ip);

        if (!$location || !$location->countryName || !$location->countryCode) {
            throw new Exception("Unable to determine location from IP: $ip");
        }
